...
function safeHttpUrl(value){try{const u=new URL(String(value||''),'https://paper-daemon.github.io/creator-stack-picker/');return ['http:','https:'].includes(u.protocol)?u.href:''}catch{return ''}}
const linkFor=t=>{const c=affiliate[t.key];const paidUrl=c&&c.enabled?safeHttpUrl(c.url):'';return paidUrl?{url:paidUrl,rel:'nofollow noopener sponsored',paid:true}:{url:t.url,rel:'noopener',paid:false}};
